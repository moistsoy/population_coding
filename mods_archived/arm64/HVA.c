...
#define t _nt->_t
#define dt _nt->_dt
#define g_hva_bar _p[0]
#define g_hva_bar_columnindex 0
#define u_hva_inf _p[1]
#define u_hva_inf_columnindex 1
#define v_hva_inf _p[2]
#define v_hva_inf_columnindex 2
#define u_hva_tau _p[3]
#define u_hva_tau_columnindex 3
#define v_hva_tau _p[4]
#define v_hva_tau_columnindex 4
#define u_hva _p[5]
#define u_hva_columnindex 5
#define v_hva _p[6]
#define v_hva_columnindex 6
#define eca _p[7]
#define eca_columnindex 7
#define Du_hva _p[8]
#define Du_hva_columnindex 8
#define Dv_hva _p[9]
#define Dv_hva_columnindex 9
#define ica _p[10]
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 static double u_hva0 = 0;
 static double v_hva0 = 0;
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {int _reset = 0; {
   Du_hva = ( u_hva_inf - u_hva ) / u_hva_tau ;
   Dv_hva = ( v_hva_inf - v_hva ) / v_hva_tau ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {
 Du_hva = Du_hva  / (1. - dt*( ( ( ( - 1.0 ) ) ) / u_hva_tau )) ;
 Dv_hva = Dv_hva  / (1. - dt*( ( ( ( - 1.0 ) ) ) / v_hva_tau )) ;
  return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
    u_hva = u_hva + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / u_hva_tau)))*(- ( ( ( u_hva_inf ) ) / u_hva_tau ) / ( ( ( ( - 1.0 ) ) ) / u_hva_tau ) - u_hva) ;
    v_hva = v_hva + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / v_hva_tau)))*(- ( ( ( v_hva_inf ) ) / v_hva_tau ) / ( ( ( ( - 1.0 ) ) ) / v_hva_tau ) - v_hva) ;
   }
  return 0;
}
 
static int  rates ( _threadargsprotocomma_ double _lv ) {
   u_hva_inf = 1.0 / ( 1.0 + exp ( - ( _lv + 24.6 ) / 11.3 ) ) ;
   v_hva_inf = 1.0 / ( 1.0 + exp ( _lv + 12.6 ) / 18.9 ) ;
   u_hva_tau = 1.25 * 2.0 / ( exp ( - 0.031 * ( _lv + 37.1 ) ) + exp ( 0.031 * ( _lv + 37.1 ) ) ) ;
   v_hva_tau = 420.0 * 3.0 ;
    return 0; }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   ica = g_hva_bar * pow( u_hva , 2.0 ) * v_hva * ( v - eca ) ;
   }
 _current += ica;
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = u_hva_columnindex;  _dlist1[0] = Du_hva_columnindex;
 _slist1[1] = v_hva_columnindex;  _dlist1[1] = Dv_hva_columnindex;
_first = 0;
}
