...
#define dt _nt->_dt
#define FARADAY _p[0]
#define FARADAY_columnindex 0
#define depth _p[1]
#define depth_columnindex 1
#define tau_ca _p[2]
#define tau_ca_columnindex 2
#define cai_inf _p[3]
#define cai_inf_columnindex 3
#define phi_ca _p[4]
#define phi_ca_columnindex 4
#define cao _p[5]
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "area");
 	...
 	ivoc_help("help ?1 cadyn /Users/ryu/Documents/Project/Population Coding/codes/mods/cadyn.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {int _reset = 0; {
   Dcai = - ( phi_ca * ica ) / ( FARADAY * ( area * depth ) ) + ( cai_inf - cai ) / tau_ca ;
   }
 return _reset;
}
 ...
 static int states (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
    cai = cai + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_ca)))*(- ( ( - ( ( phi_ca )*( ica ) ) ) / ( FARADAY * ( area * depth ) ) + ( ( cai_inf ) ) / tau_ca ) / ( ( ( ( - 1.0 ) ) ) / tau_ca ) - cai) ;
   }
  return 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/ryu/Documents/Project/Population Coding/codes/mods/cadyn.mod";
static const char* nmodl_file_text = 
  "NEURON {\n"
  "    SUFFIX cadyn\n"
